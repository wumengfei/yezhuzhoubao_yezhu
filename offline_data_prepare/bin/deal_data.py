...
rc = Redis_client(conf.redis_conf)
...
def load_showing_data():
    ...
    add_obj = open(showing_add, 'w')
    for house in showing_dict:
        str_line = house + "\t" + str(showing_dict[house]) + "\n"
        add_obj.write(str_line)
    return showing_dict
...
def list_house_this_week(showing_dict):
    ...
    for line in file_obj:
        ...
        if house_code in list_price_last_week_dict:
            ...
            list_house_dict[house_code]["list_price_last_week"] = last_list_price
            if float(total_prices) > float(last_list_price):
                list_house_dict[house_code]["list_price_qushi"] = "rise"
            elif float(total_prices) <float(last_list_price):
                list_house_dict[house_code]["list_price_qushi"] = "down"
            else:
                list_house_dict[house_code]["list_price_qushi"] = "same"
        else:
...
def deal_house_this_week(showing_dict):
    ...
    for line in file_obj:
        ...
        sold_house_dict[house_code]["build_area"] = build_size
        ...
        if showing_code in showing_dict:
            sold_house_dict[house_code]["showing"] = showing_dict[showing_code]
        else:
    ...
    if len(list_similar_list) > 0:
        ...
        try:
            for house in list_similar_list:
                ...
                result_dict[key]["list_similar"].append((house, build_size, list_interval, list_price_qushi, showing, list_price))
            result_dict[key]["list_similar"].sort(key=lambda x:x[4], reverse = True)
        except Exception, e:
            traceback.print_exc(file = err_f)
...
def weekly_report(list_house_dict, sold_house_dict):
    file_obj = open(conf.list_house, 'r')
    #file_obj = open("data/house_onsale.dat", 'r')
    ...
    try:
        for line in file_obj:
            ...
            for house in similar_list:
                ...
                elif house in list_house_dict and house != house_code:
                    list_similar_list.append(house)
                else:
                    continue
            my_log.debug("start to get result")
            result_dict = similar_house_this_week(house_code, list_house_dict, sold_house_dict, sold_similar_list, list_similar_list)
...
if __name__ == "__main__":
    try:
       ...
       list_house_dict = list_house_this_week(showing_dict)  #挂牌房源信息:上周挂牌价/挂牌价趋势/面积/挂牌价/签约时间/挂牌周期/挂牌均价/带看次数
