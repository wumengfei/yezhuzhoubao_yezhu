...
class Redis_client(object):
    ...
    def __init__(self, conf_path):
        ...
        r_host = cf.get('redis_info', 'redis_host')
        r_port = int(cf.get('redis_info', 'redis_port'))
        r_pwd = cf.get('redis_info', 'redis_pwd')
        r_db = cf.get('redis_info', 'redis_db')
        r_con_timeout = int(cf.get('redis_info', 'redis_con_timeout'))
        r_trans_timeout = int(cf.get('redis_info', 'redis_trans_timeout'))
        try:
            pool = redis.ConnectionPool(host=r_host, port=r_port, db=r_db, password=r_pwd, socket_timeout=r_trans_timeout,
                                        socket_connect_timeout=r_con_timeout)
            self.rc = redis.StrictRedis(connection_pool=pool)
        except:
            self.my_log.error("failed to connect redis server!")
            exit(1)
    ...
    def Zincrby(self, key, key_pre, subkey, amount=1):
        if len(key) == 0 or len(key_pre) == 0:
            ...
            self.my_log.debug(
                "increase the key-value score [ key: %s, value: %s score: %d]" % (real_key, subkey, amount))
